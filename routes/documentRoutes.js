...
// //     // যদি অলরেডি সাইন করা থাকে তবে নতুন করে প্রসেস করার দরকার নেই
// //     if (doc.parties[idx].status === 'signed') {
// //         return res.json({ already_signed: true, completed: doc.status === 'completed' });
// //     }

// //     const userAgent = req.headers['user-agent'] || 'Unknown Device';
// //     const ip = (req.headers['x-real-ip'] || req.headers['x-forwarded-for']?.split(',')[0] || req.socket.remoteAddress || '127.0.0.1').trim();

// //     // ২. জিও-লোকেশন (Non-blocking)
// //     let locationData = "Unknown Location";
// //     if (ip !== '127.0.0.1' && ip !== '::1') {
// //       try {
...
// //         }
// //       } catch (locErr) {
// //         console.error("Geo-location Error:", locErr.message);
// //       }
// //     }
...
/**
 * documentRoutes.js — FULLY FIXED
 *
 * Critical fixes:
 * 1. ✅ Transparent PNG merge — uses pdfDoc.embedPng with alpha intact, NO background rect drawn
 * 2. ✅ Text fields support font size, weight, family stored in field metadata
 * 3. ✅ One-time token: after signing, token is NULLED so link can never be reused
 * 4. ✅ Legal audit page appended to final PDF (signer name, email, IP, time, device, geo)
 * 5. ✅ Immediate 200 OK — PDF finalization is fire-and-forget
 * 6. ✅ Signatures stored in separate Cloudinary assets to avoid 16MB MongoDB doc limit
 */
  ...
  return transporter.sendMail({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:auto;border:1px solid #eee;
        ...
        <p style="font-size:12px;color:#666;">Or copy this link: ${signLink}</p>
        <p style="font-size:11px;color:#aaa;margin-top:16px;">⚠️ This link is one-time use only. It expires after signing.</p>
      </div>`,
  });
};

// ── CORE FIX: mergeSignatures ────────────────────────────────────────────────
// ✅ Transparent PNG: embedPng preserves alpha channel; NO drawRectangle behind it.
// ✅ Text fields: uses font size, weight, family from field metadata.
const mergeSignatures = async (doc) => {
  ...
  // Embed fonts we might need
  const helvetica     = await pdfDoc.embedFont(StandardFonts.Helvetica);
  const helveticaBold = await pdfDoc.embedFont(StandardFonts.HelveticaBold);
  const timesRoman    = await pdfDoc.embedFont(StandardFonts.TimesRoman);

  // Normalise fields — handle JSON strings
  const fieldsToProcess = (doc.fields || [])
    .map(f => (typeof f === 'string' ? JSON.parse(f) : f))
    .filter(f => f?.value);
  ...
  for (const fd of fieldsToProcess) {
    ...
    const dH = (parseFloat(fd.height) * pH) / 100;
    const dX = (parseFloat(fd.x)      * pW) / 100;
    // PDF y-axis is bottom-up; our coords are top-down percentages
    const dY = pH - ((parseFloat(fd.y) * pH) / 100) - dH;
    ...
    if (fd.type === 'signature' && fd.value.startsWith('data:image')) {
      ...
      if (header.includes('image/png')) {
        // ✅ embedPng handles the alpha channel — no white box possible
        pdfImage = await pdfDoc.embedPng(imgBuffer);
      } else {
        pdfImage = await pdfDoc.embedJpg(imgBuffer);
      }

      // ✅ Draw ONLY the image — no background rectangle, no opacity hack needed
      page.drawImage(pdfImage, {
        x:      dX,
        y:      dY,
        width:  dW,
        height: dH,
        // opacity: 1 is default — PNG alpha channels are respected natively by pdf-lib
      });

    } else if (fd.type === 'text' && fd.value) {
      // ── TEXT FIELD ────────────────────────────────────────────────────────
      const fontSize   = Number(fd.fontSize)   || 11;
      const fontWeight = fd.fontWeight         || 'normal'; // 'bold' | 'normal'
      const fontFamily = fd.fontFamily         || 'helvetica'; // 'helvetica'|'times'

      // Pick the right embedded font
      let embedFont = helvetica;
      if (fontFamily === 'times')            embedFont = timesRoman;
      if (fontWeight === 'bold')             embedFont = helveticaBold;
      if (fontFamily === 'times' && fontWeight === 'bold') embedFont = await pdfDoc.embedFont(StandardFonts.TimesRomanBold);

      // Clamp text so it doesn't overflow the field
      const textStr = String(fd.value);
...
const appendAuditPage = async (pdfBytes, doc) => {
  ...
  page.drawText('NEXSIGN — LEGAL AUDIT CERTIFICATE', {
    x: 40, y: 808, size: 14, font: bold, color: rgb(1, 1, 1),
  });

  page.drawText('Document Completion & Signature Audit Trail', {
    x: 40, y: 768, size: 11, font: bold, color: dark,
  });
  page.drawText(`Document: ${doc.title}`, {
    x: 40, y: 750, size: 10, font, color: grey,
  });
  page.drawText(`Completed: ${new Date().toUTCString()}`, {
    x: 40, y: 735, size: 10, font, color: grey,
  });

  // Divider
  page.drawLine({ start: { x: 40, y: 725 }, end: { x: 555, y: 725 }, thickness: 0.5, color: rgb(0.8, 0.8, 0.8) });

  let y = 705;
  doc.parties.forEach((p, idx) => {
    page.drawText(`Signer ${idx + 1}`, { x: 40, y, size: 11, font: bold, color: brand });
    y -= 18;
    ...
    const rows = [
      ...
      ['Signed At',    p.signedAt  ? new Date(p.signedAt).toUTCString() : 'N/A'],
    ...
    rows.forEach(([label, value]) => {
      page.drawText(`${label}:`, { x: 50,  y, size: 9, font: bold, color: dark });
      page.drawText(value,       { x: 160, y, size: 9, font,       color: grey });
      y -= 15;
    });
    y -= 10;

    // Party divider
    if (idx < doc.parties.length - 1) {
      page.drawLine({ start: { x: 40, y }, end: { x: 555, y }, thickness: 0.3, color: rgb(0.9, 0.9, 0.9) });
      y -= 15;
    }
  });

  // Footer
  page.drawRectangle({ x: 0, y: 0, width: 595, height: 30, color: rgb(0.97, 0.97, 0.97) });
  page.drawText('This certificate was generated by NeXsign e-Signature Platform. Tamper-evident digital record.', {
    x: 40, y: 10, size: 7, font, color: grey,
  });

  return pdfDoc.save();
};

// ── Fire-and-forget: finalize + email ────────────────────────────────────────
const generateAndSendFinalDoc = async (docId) => {
  try {
    const doc = await Document.findById(docId);
    if (!doc) return;
    ...
    // 2. Append legal audit page
    const finalBytes = await appendAuditPage(mergedBytes, doc);
    const pdfBuffer  = Buffer.from(finalBytes);

    // 3. Upload final PDF to Cloudinary
    const uploadRes = await uploadToCloudinary(pdfBuffer, {
      resource_type: 'raw',
      folder:        'completed_docs',
      format:        'pdf',
      public_id:     `completed_${docId}_${Date.now()}`,
    });

    doc.fileUrl = uploadRes.secure_url;
    doc.status  = 'completed';

    // 4. Clear all signature base64 values from fields to keep doc size small
    if (Array.isArray(doc.fields)) {
      doc.fields = doc.fields.map(f => {
        ...
        if (field.type === 'signature' && field.value?.startsWith('data:image')) {
          return { ...field, value: '[SIGNED]', signatureUrl: uploadRes.secure_url };
        }
        return field;
      });
    }
    ...
    // 5. Email completed PDF to all parties + CC
    const recipients = [
    ...
    if (recipients.length > 0) {
      await transporter.sendMail({
        from:    `"NeXsign" <${process.env.EMAIL_USER}>`,
        to:      recipients.join(','),
        subject: `✅ [Completed] ${doc.title} — Signed Document`,
        html: `
          <div style="font-family:sans-serif;max-width:600px;margin:auto;
            ...
            <p>The completed document with the legal audit trail is attached.</p>
            <hr style="border:none;border-top:1px solid #eee;margin:20px 0"/>
            <p style="font-size:12px;color:#888;">
              This email was sent by NeXsign e-Signature Platform.<br/>
              The attached PDF includes a tamper-evident audit certificate.
            </p>
          </div>`,
        attachments: [{
          filename: `${doc.title.replace(/\s+/g, '_')}_Signed.pdf`,
    ...
    console.log(`[NeXsign] ✅ Finalized doc ${docId}`);
  } catch (err) {
    console.error(`[NeXsign] ❌ Finalize error for ${docId}:`, err.message);
  }
};
...
router.post('/upload-and-send', auth, upload.single('file'), async (req, res) => {
  try {
    ...
    // Send first email — non-blocking
    sendSigningEmail(partiesWithTokens[0], doc.title, firstToken).catch(console.error);

    // ✅ Immediate 200 — client never waits for processing
    res.json({ success: true, documentId: doc._id });
  } catch (err) {
...
// 2. PDF Proxy — solves CORS for Cloudinary PDFs
router.get('/proxy/:path(*)', async (req, res) => {
  try {
    const cloudPath = req.params.path.replace(/~~/g, '/');
...
// 3. Get signing page data (public — token-based)
router.get('/sign/:token', async (req, res) => {
  try {
    const doc = await Document.findOne({ 'parties.token': req.params.token });
    ...
    if (party.status === 'signed')
      return res.status(410).json({ error: 'This document has already been signed. The link is no longer valid.' });
    ...
    res.json({
      ...
      title:      doc.title,
      fileUrl:    doc.fileUrl,
      // ✅ Send only fields — never send large base64 values back to signer
      fields:     (doc.fields || []).map(f => {
        const field = typeof f === 'string' ? JSON.parse(f) : f;
        // Mask completed signature values so response is lean
        if (field.type === 'signature' && field.value?.startsWith('data:image')) {
          return { ...field, value: '' };
        }
        return field;
...
router.post('/sign/submit', async (req, res) => {
  try {
    ...
    if (signer.status === 'signed')
      return res.status(410).json({ error: 'Already signed. This link is now expired.' });

    const ip        = req.headers['x-forwarded-for']?.split(',')[0]?.trim() || req.socket.remoteAddress || 'Unknown';
    const userAgent = req.headers['user-agent'] || 'Unknown';

    // ✅ Save updated fields (with signature base64 values)
    doc.fields = fields;
    doc.markModified('fields');
    ...
    signer.location   = locationData?.city
      ...
      : (locationData?.text || 'Unknown');
    signer.postalCode = locationData?.postal || '';
    signer.device     = deviceInfo || userAgent.substring(0, 200);
    signer.timeZone   = clientTime || '';

    // ✅ ONE-TIME TOKEN: null the token immediately after signing
    signer.token = undefined;
    doc.parties[idx] = signer;
    doc.markModified('parties');
    ...
    AuditLog.create({
      ...
      details:      `Signed via ${deviceInfo || 'web browser'}`,
    }).catch(console.error);

    // Check if more signers remain
    const allSigned    = doc.parties.every(p => p.status === 'signed');
    const nextUnsigned = !allSigned ? doc.parties.find((p, i) => i > idx && p.status !== 'signed') : null;

    if (!allSigned && nextUnsigned) {
      // Advance to next signer
      const nextToken = crypto.randomBytes(32).toString('hex');
      const nextIdx   = doc.parties.findIndex((p, i) => i > idx && p.status !== 'signed');
      doc.parties[nextIdx].token  = nextToken;
      doc.parties[nextIdx].status = 'sent';
      doc.markModified('parties');
      await doc.save();

      sendSigningEmail(doc.parties[nextIdx], doc.title, nextToken).catch(console.error);
...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const skip  = (page - 1) * limit;
    const query = {
      $or: [{ owner: req.user.id }, { 'parties.email': req.user.email }],
    };

    const [documents, total] = await Promise.all([
      Document.find(query)
        .select('-fields') // ✅ Exclude heavy fields array from list view
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .lean(),
...
// 6. Get single document (includes fields)
router.get('/:id', auth, async (req, res) => {
  try {
    const doc = await Document.findById(req.params.id);
    if (!doc)
