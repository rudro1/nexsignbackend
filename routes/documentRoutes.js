...
// module.exports = router; workable
...
/**
 * ২. সিগনেচার মার্জিং লজিক (Scalable & Memory Efficient)
 */
const mergeSignatures = async (doc) => {
  try {
    const response = await axios.get(doc.fileUrl, { 
    ...
    // ইউনিক ফিল্ড হ্যান্ডলিং - ডাটা টাইপ চেক সহ
    const uniqueFields = Array.from(new Map((doc.fields || []).map(f => {
      try {
        const obj = typeof f === 'string' ? JSON.parse(f) : f;
        return [obj.id, obj];
      } catch (e) { return [null, null]; }
    })).values()).filter(f => f && f.id);
    ...
    for (const fd of uniqueFields) {
      ...
      // কোঅর্ডিনেট ক্যালকুলেশন ফিক্স
      const drawW = (parseFloat(fd.width) * width) / 100;
      const drawH = (parseFloat(fd.height) * height) / 100;
      const drawX = (parseFloat(fd.x) * width) / 100;
      const drawY = height - ((parseFloat(fd.y) * height) / 100) - drawH;

      if (typeof fd.value === 'string' && fd.value.startsWith('data:image')) {
        try {
          const base64Data = fd.value.split(',')[1];
          const imgBuffer = Buffer.from(base64Data, 'base64');
          ...
          page.drawImage(sigImg, { 
            x: drawX, y: drawY, width: drawW, height: drawH, opacity: 1 
          });
    ...
    console.error("Merge Signatures Error:", err);
    throw err;
  }
};

/**
 * ৩. অডিট সার্টিফিকেট জেনারেশন
 */
const generateAndSendFinalDoc = async (doc) => {
  try {
    const basePdfBytes = await mergeSignatures(doc);
    ...
    // সার্টিফিকেট হেডার ডিজাইন
    page.drawRectangle({ x: 20, y: 20, width: width - 40, height: height - 40, borderColor: rgb(0.8, 0.8, 0.8), borderWidth: 1 });
    page.drawRectangle({ x: 21, y: y - 50, width: width - 42, height: 60, color: brandColor });
    page.drawText('NEXSIGN DIGITAL AUDIT CERTIFICATE', { x: 50, y: y - 35, size: 18, font: boldFont, color: rgb(1, 1, 1) });
    y -= 100;

    const drawInfo = (label, value) => {
      page.drawText(label, { x: 50, y, size: 11, font: boldFont });
      page.drawText(String(value || 'N/A'), { x: 160, y, size: 11, font: timesFont });
      y -= 22;
    };

    drawInfo('Document Title:', doc.title);
    drawInfo('Created By:', `${doc.senderMeta?.name || 'System'} (${doc.senderMeta?.email || 'N/A'})`);
    drawInfo('Initiated At:', doc.senderMeta?.time || new Date(doc.createdAt).toLocaleString());
    y -= 20;

    page.drawText('SIGNER DETAILS & AUDIT TRAIL', { x: 50, y, size: 13, font: boldFont, color: brandColor });
    y -= 25;

    for (const [index, p] of doc.parties.entries()) {
      if (y < 150) { page = pdfDoc.addPage([600, 850]); y = height - 50; }
      ...
      page.drawText(`Email: ${p.email} | IP: ${p.ipAddress || 'N/A'}`, { x: 70, y, size: 9, font: timesFont });
      y -= 15;
      const locStr = `${p.location || 'Unknown'} ${p.postalCode ? `(Zip: ${p.postalCode})` : ''}`;
      page.drawText(`Location: ${locStr}`, { x: 70, y, size: 9, font: timesFont });
      y -= 15;
      
      const signedTime = p.signedAt ? new Date(p.signedAt).toLocaleString('en-US', { 
        timeZone: 'Asia/Dhaka', dateStyle: 'medium', timeStyle: 'medium' 
      }) : 'N/A';
      page.drawText(`Signed At: ${signedTime} (BST)`, { x: 70, y, size: 9, font: boldFont });
      y -= 35; 
    }
    ...
    // ক্লাউডিনারি আপলোড (Overwrite true যাতে ডুপ্লিকেট না হয়)
    const uploadResult = await new Promise((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        { resource_type: "raw", folder: "completed_docs", public_id: `final_${doc._id}`, overwrite: true },
        (err, res) => err ? reject(err) : resolve(res)
      );
      stream.end(pdfBuffer);
    });
    ...
    // ইমেইল লজিক - সলিড প্রাপক লিস্ট
    const allRecipients = [...new Set([
      ...doc.parties.map(p => p.email), 
      ...(doc.ccEmails || []),
      doc.senderMeta?.email
    ...
    if (allRecipients.length > 0) {
      await transporter.sendMail({
        ...
        to: allRecipients.join(','), 
        subject: `Completed: ${doc.title}`,
        html: `<h3>Successfully Executed!</h3><p>The document <b>${doc.title}</b> has been signed by all parties. Find the completed PDF attached.</p>`,
        attachments: [{ filename: `${doc.title.replace(/\s+/g, '_')}_Final.pdf`, content: pdfBuffer }]
      });
    }
  } catch (err) { console.error("Finalize Error:", err); }
};
...
router.post('/sign/submit', async (req, res) => {
  try {
    ...
    try {
      if (ip !== '127.0.0.1' && ip !== '::1') {
        const locRes = await axios.get(`http://ip-api.com/json/${ip}?fields=status,country,regionName,city,zip`, { timeout: 3000 });
        if (locRes.data?.status === 'success') {
          locationData = `${locRes.data.city}, ${locRes.data.regionName}, ${locRes.data.country}`;
          postalCode = locRes.data.zip || "N/A";
        }
      }
    } catch (e) { console.error("Geo API error"); }

    const bstTime = new Date().toLocaleString('en-US', { timeZone: 'Asia/Dhaka' });

    // ডাটাবেস আপডেট (Atomic way)
    doc.fields = incomingFields;
    doc.parties[idx].status = 'signed';
    doc.parties[idx].signedAt = new Date();
    doc.parties[idx].ipAddress = ip;
    doc.parties[idx].location = locationData;
    doc.parties[idx].postalCode = postalCode;
    doc.parties[idx].device = userAgent;

    await AuditLog.create({
      document_id: doc._id,
      action: 'signed',
      performed_by: { name: doc.parties[idx].name, email: doc.parties[idx].email },
      ip_address: ip,
      user_agent: userAgent,
      location: locationData,
      postal_code: postalCode,
      client_time: bstTime,
      details: `Signed from ${userAgent}`
    });
    ...
    if (idx + 1 < doc.parties.length) {
      // পরবর্তী সাইনারের জন্য টোকেন জেনারেট
      const nextToken = crypto.randomBytes(32).toString('hex');
      doc.parties[idx + 1].token = nextToken;
      doc.parties[idx + 1].status = 'sent';
      doc.currentPartyIndex = idx + 1;
      ...
      // এখানে আপনি চাইলে Next Signer-কে ইমেইল পাঠানোর লজিক ট্রিগার করতে পারেন
      return res.json({ next: true });
    } else {
      doc.status = 'completed';
      await doc.save();
      
      // ব্যাকগ্রাউন্ডে পিডিএফ জেনারেশন শুরু
      setImmediate(() => generateAndSendFinalDoc(doc));
      return res.json({ completed: true });
    }
  } catch (err) { 
    console.error(err);
